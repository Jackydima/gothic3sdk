...
GEInt MagicAimDeactivate(gCScriptProcessingUnit *a_pSPU, Entity *a_pSelfEntity, Entity *a_pOtherEntity, GEInt p_args)
{
    ...
    if (Self.IsPlayer()
        && !Self.Routine.GetCurrentTask().Contains(
            "PS_Magic") /* && !Self.Routine.GetCurrentTask ( ).Contains ( "PS_Ranged" )*/)
    {
        crossHair.AnimateWindow(500, 0x90000);
    }
    return Hook_MagicAimDeactivate.GetOriginalFunction(&MagicAimDeactivate)(a_pSPU, a_pSelfEntity, a_pOtherEntity,
                                                                            p_args);
}
...
void InitGUI()
{
    ...
    Hook_CFFGFCWnd_OnPaint.Prepare(RVA_Generic("GFC.dll", 0x4680), &CFFGFCWnd_OnPaint, mCBaseHook::mEHookType_ThisCall)
        .Hook();
    Hook_MagicAimDeactivate.Prepare(RVA_ScriptGame(0xb0ef0), &MagicAimDeactivate).Hook();
    Hook_MagicAim.Prepare(RVA_ScriptGame(0x79e20), &MagicAim).Hook();
}

void DoIt(CFFGFCWnd *a0)
{
    if (!a0 || !gCSession::GetSession().IsValid() || gCSession::GetSession().IsPaused()
        || !gCSession::GetSession().GetGUIManager())
    {
        return;
    }

    if (gCSession::GetInstance().GetGUIManager()->IsMenuOpen()
        || gCSession::GetInstance().GetGUIManager()->IsAnyPageOpen() || gCInfoManager_PS::GetInstance()->IsRunning)
    {
        return;
    }
